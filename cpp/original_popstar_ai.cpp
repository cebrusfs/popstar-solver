...
class Game {
  ...
  public:
    ...
    void parallel_solve(int limit)
    {
        ...
        {
            const auto& g = gs[i]; // gs[i] is a COLOR_GROUP (pair<int, GROUP>)
            assert(g.second.size() >= 2);

            Game temp_game = *this; // Make a copy for this thread

            temp_game.eliminate(g.second);
            temp_game.solve(1, limit); // solve returns score *from the new state onwards*
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        int real_depth = calc_real_depth(game.step, depth);
